...
//            $result['totalCRewardMoney'] = $cbillInfo->reward_money + 0;
            ...
            $return_res['msg'] = [
                ...
                'CRate'=>$result['payRate']==0?0:
                    round($result['unpayRate']/$result['payRate'],3)*100,
                'avgRate'=>$avgRate,
...
//            $AlreadyMoney=PeopleMoneyExchangeBill::find()->select(['exchange_money'])->where(['user_id'=>$user_id])->sum('exchange_money');
            ...
            $query='SELECT `reward_money`, `create_time` FROM `refree_settle_bill` ORDER BY create_time desc limit '.$pagestart.','.$pagesize;
...
//
            if($money < 1 || $money > $withdrawMoney)
            {
                $return_res['code'] = '1008';
                $return_res['results'] = "money error";
                $return_res['msg'] = "提现金额必须大于1且小于可提现金额。";
                echo json_encode($return_res);
                Yii::$app->end();
            }
